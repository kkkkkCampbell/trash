...
# MAX_QUICK_STRATEGIES=5 ./blockcheck.sh
...
# PARALLEL=0 MAX_QUICK_STRATEGIES=2 ./blockcheck.sh

###########################################################

echo
echo "BLOCKCHECK PATCHED VERSION"
echo
...
DOMAINS_DEFAULT=${DOMAINS_DEFAULT:-rr15---sn-n8v7kne6.googlevideo.com}
QNUM=${QNUM:-59780}
...
MAX_QUICK_STRATEGIES=${MAX_QUICK_STRATEGIES:-1}
QUICK_FOUND_PER_TEST=0
...
quick_test_limit_reached()
{
    [ "$SCANLEVEL" != quick ] && return 1
    [ "$QUICK_FOUND_PER_TEST" -ge "$MAX_QUICK_STRATEGIES" ] && return 0
    return 1
}
...
increment_quick_counter()
{
    [ "$SCANLEVEL" = quick ] && QUICK_FOUND_PER_TEST=$((QUICK_FOUND_PER_TEST + 1))
}
...
pktws_check_domain_http_bypass_()
{
	...
	[ "$sec" = 0 ] && {
		for s in '--hostcase' '--hostspell=hoSt' '--hostnospace' '--domcase' '--methodeol'; do
			pktws_curl_test_update $1 $3 $s && quick_test_limit_reached && return
		done
	}
	...
	for e in '' '--wssize 1:6'; do
		...
		[ -n "$e" ] && {
			pktws_curl_test_update $1 $3 $e && quick_test_limit_reached && return
		}
		...
		for desync in multisplit multidisorder; do
			...
			for pos in $splits; do
				pktws_curl_test_update $1 $3 --dpi-desync=$desync --dpi-desync-split-pos=$pos $e && {
					quick_test_limit_reached && return
					ok=1
					need_wssize=0
		...
		for desync in syndata ${need_split:+syndata,multisplit} ${need_disorder:+syndata,multidisorder} ; do
			pktws_curl_test_update_vary $1 $2 $3 $desync $e && quick_test_limit_reached && return
			pktws_curl_test_update_vary $1 $2 $3 $desync --dpi-desync-fake-syndata="$ZAPRET_BASE/files/fake/$s" $e && quick_test_limit_reached && return
		done
...
pktws_check_domain_http3_bypass_()
{
	...
	[ "$IPV" = 6 ] && {
		...
		for desync in $f; do
			pktws_curl_test_update $1 $2 --dpi-desync=$desync && quick_test_limit_reached && return
		done
	}
	...
	[ "$UNAME" != "OpenBSD" ] && [ "$IPV" = 4 -o -n "$IP6_DEFRAG_DISABLE" ] && {
		for frag in 8 16 24 32 40 64; do
			...
			for desync in $tests; do
				pktws_curl_test_update $1 $2 --dpi-desync=$desync --dpi-desync-ipfrag-pos-udp=$frag && quick_test_limit_reached && return
			done
		done
	}
...
tpws_check_domain_http_bypass_()
{
	...
	if [ "$sec" = 0 ]; then
		for s in '--hostcase' '--hostspell=hoSt' '--hostdot' '--hosttab' '--hostnospace' '--domcase' ; do
			tpws_curl_test_update $1 $3 $s && quick_test_limit_reached && return
		done
...
ask_params()
{
	...
	[ -n "$DOMAINS" ] || {
		...
		[ "$BATCH" = 1 ] || {
			...
			# read dom
			dom=rr15---sn-n8v7kne6.googlevideo.com
			[ -n "$dom" ] && DOMAINS="$dom"
		}
	}
	...
	[ -n "$ENABLE_HTTP" ] || {
		...
		[ "$BATCH" = 1 ] || {
			echo
			# JOPA ask_yes_no_var ENABLE_HTTP "check http"
		}
	}
	...
	[ -n "$ENABLE_HTTPS_TLS12" ] || {
		...
		[ "$BATCH" = 1 ] || {
			echo
			# JOPA ask_yes_no_var ENABLE_HTTPS_TLS12 "check https tls 1.2"
		}
	}
	...
	[ -n "$ENABLE_HTTPS_TLS13" ] || {
		...
		if [ -n "$TLS13" ]; then
			[ "$BATCH" = 1 ] || {
				...
				# JOPA ask_yes_no_var ENABLE_HTTPS_TLS13 "check https tls 1.3"
			}
		else
			echo
	...
	[ -z "$PARALLEL" -a $REPEATS -gt 1 ] && {
		...
		[ "$BATCH" = 1 ] || {
			...
			# ask_yes_no_var PARALLEL "enable parallel scan"  # <-- Закомментировать
			PARALLEL=1  # <-- Добавить принудительную установку
			echo "Parallel scan enabled by default"
		}
	}
	PARALLEL=${PARALLEL:-0}
	...
	[ -n "$SCANLEVEL" ] || {
		...
		[ "$BATCH" = 1 ] || {
			...
			# JOPA ask_list SCANLEVEL "quick standard force" "$SCANLEVEL"
			# disable tpws checks by default in quick mode
			[ "$SCANLEVEL" = quick -a -z "$SKIP_TPWS" -a "$UNAME" != Darwin -a "$MAX_QUICK_STRATEGIES" -le 1 ] && SKIP_TPWS=1
		}
	}
